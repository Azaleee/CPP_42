...
void Bureaucrat::executeForm(AForm const &form) const
{
	...
	{
		form.execute(*this);
		std::cout << "[" << this->getName() << "] exectued \"" << form.getName() << "\"" << std::endl;
	}
	catch(const std::exception &e)
	{
		// std::cout << "[" << this->getName() << "] could'nt execute \"" << form.getName() << "\" because grade " << this->getGrade() << " is lower than grade to execute " << form.getGradeToExecute() << std::endl;
